...
audio engine bindings). This plugin:
  ...
  1. Parses the game tone chains from PSARC/sloppak files.
...
# the game gear photos extracted by extract_gear_photos.py live in
...
def _find_gear_photo(rs_gear: str) -> Path | None:
    """Locate the PNG for a game gear if extract_gear_photos.py
...
def _load_rs_cab_to_ir() -> dict:
    ...
    extract_irs.py. Each key is a game gear entity (e.g.
...
def _enrich_chain_piece(piece: dict, img_idx: dict | None = None) -> dict:
    ...
    # (e.g. a user that doesn't own the game and so hasn't run
...
def _wire_cabs_to_presets(replace_auto: bool = False) -> dict:
    ...
    the newly-extracted the game IRs now". Manual overrides are
        ...
        for idx, song_path in enumerate(songs):
            ...
            for tone in tones:
                ...
                for piece in parsed["chain"]:
                    ...
                    # Cabs first: if we extracted a game IR for
...
def setup(app, context):
    ...
    def status():
        ...
        # gears.psarc. A fresh install (or a user without the game)
    ...
    def gear_catalog():
        ...
        for gear, b in best.items():
            ...
            cats.setdefault(category, []).append({
                ...
                # the game does (which is the order they appear in
    ...
    def nam_inventory():
        ...
        # weak hint — `extract_irs.py` puts every the game cab IR into
    ...
    def gear_photo(rs_gear: str):
        """Serve the game-extracted photo for `rs_gear` (PNG).
