...
protected:
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i = 0; i < frames; ++i) { oL[i] = rbAmpLvl(kGa79Lvl * softClip(kGa79Makeup * L.process(iL[i])) * 0.98f); oR[i] = rbAmpLvl(kGa79Lvl * softClip(kGa79Makeup * R.process(iR[i])) * 0.98f); }
    }
