/*
 * Hipzon GA-79 RVT — Gibson GA-79 RVT Multi Stereo, COMPONENT-LEVEL model.
 *
...
struct Tremolo {
    ...
    inline float process(float x, float rateHz, float depth){
        ...
        const float lfo = 0.5f * (1.f + (float)std::sin(ph));
...
static constexpr float kGa79Makeup = 3.50f;   // tuned offline (-14 dBFS @ kDef)
static constexpr float kGa79Lvl    = 0.3352f;

class Ga79Plugin : public Plugin {
    Ga79Channel L, R;
...
public:
    Ga79Plugin() : Plugin(kParamCount, 0, 0) {
        for (int i = 0; i < kParamCount; ++i) fParams[i] = kGa79Def[i];
        const float sr = (float)getSampleRate();
        L.setSampleRate(sr); R.setSampleRate(sr); L.reset(); R.reset(); recalc();
    }
protected:
    const char* getLabel()       const override { return "HipzonGA79RVT"; }
    const char* getDescription() const override { return "Gibson GA-79 RVT stereo guitar amp (reverb + tremolo) — component-level model"; }
    const char* getMaker()       const override { return "RigBuilder"; }
    const char* getLicense()     const override { return "ISC"; }
    uint32_t    getVersion()     const override { return d_version(1, 0, 0); }
    int64_t     getUniqueId()    const override { return d_cconst('R', 'B', '7', '9'); }

    void initParameter(uint32_t i, Parameter& p) override {
        if (i >= (uint32_t)kParamCount) return;
        p.hints = kParameterIsAutomatable;
        p.name = kGa79Names[i]; p.symbol = kGa79Symbols[i];
        p.ranges.min = kGa79Min[i]; p.ranges.max = kGa79Max[i]; p.ranges.def = kGa79Def[i];
    }
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i = 0; i < frames; ++i) { oL[i] = rbAmpLvl(kGa79Lvl * softClip(kGa79Makeup * L.process(3.2f * iL[i])) * 0.98f); oR[i] = rbAmpLvl(kGa79Lvl * softClip(kGa79Makeup * R.process(3.2f * iR[i])) * 0.98f); }
    }
    DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(Ga79Plugin)
};

Plugin* createPlugin() { return new Ga79Plugin(); }
