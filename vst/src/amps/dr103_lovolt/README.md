# LovoltDR103 — "Lovolt DR103"

White-box model of the **Hiwatt DR103 "Custom Hiwatt 100"** for Rocksmith's
`Amp_HG100` (the gear map's generic name is "Peavey 5150", but the folder +
schematic are the Hiwatt DR103). Parody brand **"Lovolt"** (Hiwatt = high watt
-> Lovolt = low volt; the same brand as the Lovolt 100). The face must never
read "Hiwatt".

Modelled component-by-component from `amps/Hiwatt DR103 (HG500)/DR103_Complete.pdf`.
...
A high-headroom, clean-and-loud EL34 amp (3× ECC83 + ECC81 PI + 2× EL34 ~100W).
...
## Build
```
make DPF_PATH=/tmp/dpf BUILD_DIR=/tmp/dr103_build TARGET_DIR=/tmp/dr103_out vst3
```
Copy into `vst/amps/LovoltDR103.vst3/Contents/MacOS/LovoltDR103` and
`codesign --force --deep --sign - LovoltDR103.vst3`. UNIQUE_ID `Lv50`.
