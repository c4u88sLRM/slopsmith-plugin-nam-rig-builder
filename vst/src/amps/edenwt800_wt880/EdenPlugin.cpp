 ...
 * head), COMPONENT-LEVEL model.
 *
 * Built from the Eden WT-800C factory schematic:
 *   • INPUT   — 12AX7 (V1B) twin-triode input stage (Valve-Tech warmth)
 *   • GAIN/COMP — TL072 gain (VR1) + the Eden opto compressor (VR2 Comp Level)
 *   • ENHANCE — TL072 contour (VR4): scoops mids, lifts lows + highs
 *   • BASS/TREBLE — VR5/VR6 shelves
 *   • SEMI-PARAMETRIC EQ — Low/Mid/High (VR7/8/9 level + VR101/2/3 dual freq):
 *     30-300 / 200-2k / 1.2-12k Hz, +/-15 dB
 *   • MASTER  — VR11 output level
 *   • BI-AMP  — Crossover Freq (VR12), Balance (VR13), Crossover On/Off (S4),
 *               Bridge/Mono — the WT-880's bi-ampable power section
 *
 * Same Eden preamp engine as the Aiden GT-300/550 (nodal 12AX7 + clean TL072 EQ
 * + Eden Enhance/Comp); the WT-880 adds the bi-amp crossover (a first-order
 * complementary low/high split tilted by Balance — lo+hi reconstructs flat).
 */
...
class EdenChannel {
    ...
    Triode v1;
    OptoComp comp;
    Biquad enhMid, enhLow, enhHigh;
    Biquad bqBass, bqTreble;
    Biquad band[3];
    float drive=1, master=1, compAmt=0;
    // bi-amp crossover (one-pole complementary split + Balance tilt)
    float xa=0.05f, xlp=0.f, gLow=1, gHigh=1; bool xoverOn=false;
public:
    void setSampleRate(float s){ fs=(s>0)?s:48000.f; v1.setT(s); comp.setSR(fs); }
    void reset(){ v1.reset(); comp.reset(); xlp=0.f;
    ...
    void setParams(const float* p) {
        ...
        bqBass.setLowShelf(50.f, (p[kBass]-0.5f)*30.f, fs);
        ...
        for (int i=0;i<3;++i){
            ...
            band[i].setPeak(fc, (p[lv[i]]-0.5f)*30.f, 0.8f, fs);
        }
        master = p[kMaster] / 0.7f;

        // bi-amp crossover: 100 Hz .. 5 kHz (log), Balance tilts lows vs highs.
        xoverOn = p[kXoverOn] > 0.5f;
        const float fc = 100.f * std::pow(50.0f, p[kXoverFreq]);    // 100..5000 Hz
        xa = 1.f - std::exp(-6.2831853f * fc / fs);
    ...
    inline float process(float x) {
        ...
        s = comp.process(s, compAmt);
        s = enhMid.process(s); s = enhLow.process(s); s = enhHigh.process(s);
        s = bqBass.process(s);
        s = band[0].process(s); s = band[1].process(s); s = band[2].process(s);
        s = bqTreble.process(s);
        if (xoverOn) {                       // bi-amp: complementary split, Balance tilt
            xlp += xa * (s - xlp);
            const float hi = s - xlp;
            s = xlp * gLow + hi * gHigh;
        }
        return s * master;
    }
...
protected:
    ...
    const char* getDescription() const override { return "Eden WT-880 World Tour Valve-Tech bass head — component-level model"; }
