 ...
 *
 * Built from the SWR Red Head Preamp schematic:
 *   • INPUT  — Passive / Active → 12AX7 input stage
 *   • GAIN   — 12AX7 drive (a Pull Turbo diode network = hotter/grittier)
 *   • AURAL ENHANCER — SWR's variable "smile" contour (lifts lows + highs,
 *              scoops the low-mids)
 *   • TONE   — Bass shelf, semi-parametric Mid (Level + sweepable Freq), Treble
 *              shelf (Pull Transparency = extra high air); TL072 EQ
 *   • POWER  — 350 W solid-state (clean, hi-fi; the only nonlinearity is the
 *              12AX7 + Turbo)
 *
 * The 12AX7 is the REAL nodal triode (Koren + Newton/sample); the EQ is white-
 * boxed from the schematic's 50k/470k pots. The SWR voice is clean and scooped.
 */
...
// ── RBJ biquad — Aural Enhancer + tone stack ─────────────────────────────────
class Biquad {
...
struct Triode {
    ...
    void reset() { vG=0; vP=200; vK=1.4; dcAvg=200.0;  for (int i=0;i<2000;++i) process(0.0); }
...
class RedheadChannel {
    float fs = 48000.f;
    Triode v1;
    Biquad auralLo, auralMid, auralHi;        // Aural Enhancer contour
    Biquad bqBass, bqMid, bqTreble, transHi;  // tone stack + Transparency
    float drive=1, master=1; bool turbo=false, transp=false;
public:
    void setSampleRate(float s){ fs=(s>0.f)?s:48000.f; v1.setT(s); }
    void reset(){ v1.reset(); auralLo.reset(); auralMid.reset(); auralHi.reset();
        bqBass.reset(); bqMid.reset(); bqTreble.reset(); transHi.reset(); }

    void setParams(const float* p) {
        const float pad = (p[kActive] > 0.5f) ? 0.45f : 1.0f;
        turbo = p[kTurbo] > 0.5f;
        drive = (0.4f + p[kGain] * 7.0f) * (turbo ? 1.6f : 1.0f) * pad;

        // Aural Enhancer: lift lows + highs, scoop the low-mids (depth = knob).
        const float e = p[kAural];
        auralLo.setLowShelf(80.f, e*5.0f, fs);
        auralMid.setPeak(250.f, -e*6.0f, 0.8f, fs);
        auralHi.setHighShelf(3000.f, e*5.0f, fs);

        bqBass.setLowShelf(80.f, (p[kBass]-0.5f)*30.f, fs);
        const float midFc = 100.f * std::pow(20.0f, p[kMidFreq]);     // 100 Hz .. 2 kHz
        bqMid.setPeak(midFc, (p[kMidLevel]-0.5f)*30.f, 0.8f, fs);
        bqTreble.setHighShelf(5000.f, (p[kTreble]-0.5f)*30.f, fs);
        transp = p[kTransparency] > 0.5f;
        if (transp) transHi.setHighShelf(9000.f, 5.0f, fs); else transHi.setBypass();

        master = p[kMaster] / 0.7f;
    }

    inline float process(float x) {
        float s = (float)v1.process((double)(drive * x));            // 12AX7 input (Gain)
        if (turbo) s = std::tanh(s * 1.3f) * 0.769f;                 // Pull Turbo grit
        s = auralLo.process(s); s = auralMid.process(s); s = auralHi.process(s);  // Aural Enhancer
        s = bqBass.process(s); s = bqMid.process(s); s = bqTreble.process(s); s = transHi.process(s);
        return s * master;                                          // SS power (clean)
    }
};
...
protected:
    ...
    const char* getDescription() const override { return "SWR Super Redhead bass preamp — component-level model"; }
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i=0;i<frames;++i){ oL[i]=rbAmpLvl(kRedheadLvl*softClip(kRedheadMakeup*L.process(iL[i]))*0.98f); oR[i]=rbAmpLvl(kRedheadLvl*softClip(kRedheadMakeup*R.process(iR[i]))*0.98f); }
    }
