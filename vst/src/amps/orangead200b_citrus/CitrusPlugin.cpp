/*
 * Citrus AD200 — Orange AD200B (MK3) all-tube bass head, COMPONENT-LEVEL model.
 *
 * Built from the AD200B MK3 panel + documented circuit (2x ECC83 + 1x ECC81
 * driver preamp, 4x KT88/6550 power, ~200 W):
 *   • INPUT  — Passive / Active jacks (Active pads hot active basses)
 *   • V1 ECC83 (12AX7) input gain stage (Gain)
 *   • TONE   — Orange PASSIVE SUBTRACTIVE tone stack: Bass/Middle/Treble are flat
 *              at full up (1.0) and CUT as you turn down (per the manual)
 *   • ECC81 phase inverter -> 4x KT88 push-pull (~200 W): big clean headroom,
 *              creamy power-tube overdrive only when Master is really pushed
 *
 * "the simplest and purest bass amplifier" — short signal path, so the model is
 * a real nodal 12AX7 (Koren + Newton/sample) into a white-boxed subtractive
 * stack and a late-knee KT88 power stage.
 */
...
struct Triode {
    ...
    inline double process(double vin) {
        const double Bp=320, Rp=100000, Rk=1500, h=1e-4;       // Orange runs a stiff, high B+
...
class CitrusChannel {
    float fs = 48000.f;
    Triode v1;
    Biquad bqBass, bqMid, bqTreble, pwrLP;
    float drive=1, master=1, pwrDrive=1;

    // 4x KT88 push-pull (~200 W): symmetric soft clip with a LATE knee — the
    // AD200B stays clean far up, then breaks into creamy power-tube overdrive.
    static inline float pushPull(float x) {
        return std::tanh(x * 0.62f) * 1.6129f;    // 1/0.62 make-up so small x ≈ unity
    }
public:
    ...
    void setParams(float gain, float bass, float middle, float treble, float masterP, bool active) {
        ...
        // Orange SUBTRACTIVE tone stack: dB = (knob - 1) * range, so full up = 0 dB
        // (flat) and lower = cut only.
        bqBass.setLowShelf (110.f,  (bass   - 1.0f) * 20.f, fs);
        bqMid.setPeak      (550.f,  (middle - 1.0f) * 10.f, 0.9f, fs);   // 1k pot -> subtle mid
        bqTreble.setHighShelf(3500.f,(treble - 1.0f) * 20.f, fs);

        master   = masterP / 0.7f;
        pwrDrive = 0.5f + master * 0.85f;
        pwrLP.setLowpassQ(9000.f, 0.7f, fs);              // KT88 + output transformer band-limit
    }
...
class CitrusPlugin : public Plugin {
    ...
    void recalc() {
        const bool act = fParams[kActive] > 0.5f;
        L.setParams(fParams[kGain], fParams[kBass], fParams[kMiddle], fParams[kTreble], fParams[kMaster], act);
        R.setParams(fParams[kGain], fParams[kBass], fParams[kMiddle], fParams[kTreble], fParams[kMaster], act);
    }
public:
...
protected:
    ...
    const char* getDescription() const override { return "Orange AD200B all-tube bass head — component-level model"; }
