/* Epicall Ruby UI — Epiphone Electar Zephyr style vintage panel. (The in-app
 * face is the canvas spec in pedal_canvas.js; this native UI is only for
 * standalone/host use.) Knobs vertical-drag. */
...
static const Spot kKnobs[] = {
    ...
    { kBass,   0.560f, 0.60f, 0.050f, "BASS" },
    { kTreble, 0.720f, 0.60f, 0.050f, "TREBLE" },
};
static const int kNumKnobs = (int)(sizeof(kKnobs)/sizeof(kKnobs[0]));

class RubyUI : public UI {
    ...
    void drawKnob(const Spot& k){
        ...
        beginPath(); circle(cx,cy,R); fillColor(Color(232,228,214)); fill();          // cream pointer knob
        ...
        strokeColor(Color(40,36,30)); strokeWidth(2.6f*f); stroke();
        textAlign(ALIGN_CENTER|ALIGN_TOP); fontSize(10*f); fillColor(Color(40,36,30));
        text(cx,cy+R+6*f,k.name,NULL);
    }

    void onNanoDisplay() override {
        const float w=W(), h=H(), f=scale();
        beginPath(); rect(0,0,w,h); fillColor(Color(120,80,44)); fill();              // walnut cabinet
        beginPath(); rect(w*0.10f,h*0.15f,w*0.80f,h*0.70f); fillColor(Color(196,192,180)); fill(); // silver panel
        fontSize(24*f); fillColor(Color(40,36,30)); textAlign(ALIGN_CENTER|ALIGN_MIDDLE);
        text(w*0.5f,h*0.28f,"Epicall  Ruby",NULL);
...
public:
    RubyUI() : UI(900,300), fDrag(-1), fLastY(0), fDragVal(0) {
        for (int i=0;i<kParamCount;++i) fValues[i]=kRubyDef[i];
    }
protected:
    void parameterChanged(uint32_t i, float v) override { if (i<(uint32_t)kParamCount){ fValues[i]=v; repaint(); } }
    DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(RubyUI)
};

UI* createUI() { return new RubyUI(); }
