 ...
 *
 * Real nodal 12AX7s (Koren + Newton/sample) + RBJ tone + an EL84 push-pull
 * saturator. Shared MNA/Triode/Biquad blocks are byte-identical to Citrus AD200.
 */
...
static constexpr float kGa88Lvl    = 0.3930f;
...
public:
    Ga88Plugin() : Plugin(kParamCount, 0, 0) {
        for (int i = 0; i < kParamCount; ++i) fParams[i] = kGa88Def[i];
        const float sr = (float)getSampleRate();
        L.setSampleRate(sr); R.setSampleRate(sr); L.reset(); R.reset(); recalc();
    }
protected:
    const char* getLabel()       const override { return "HipzonGA88"; }
    const char* getDescription() const override { return "Gibson GA-88S stereo guitar amp — component-level model"; }
    const char* getMaker()       const override { return "RigBuilder"; }
    const char* getLicense()     const override { return "ISC"; }
    uint32_t    getVersion()     const override { return d_version(1, 0, 0); }
    int64_t     getUniqueId()    const override { return d_cconst('R', 'B', '8', '8'); }

    void initParameter(uint32_t i, Parameter& p) override {
        if (i >= (uint32_t)kParamCount) return;
        p.hints = kParameterIsAutomatable;
        p.name = kGa88Names[i]; p.symbol = kGa88Symbols[i];
        p.ranges.min = kGa88Min[i]; p.ranges.max = kGa88Max[i]; p.ranges.def = kGa88Def[i];
    }
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i = 0; i < frames; ++i) { oL[i] = rbAmpLvl(kGa88Lvl * softClip(kGa88Makeup * L.process(3.2f * iL[i])) * 0.98f); oR[i] = rbAmpLvl(kGa88Lvl * softClip(kGa88Makeup * R.process(3.2f * iR[i])) * 0.98f); }
    }
    DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(Ga88Plugin)
};

Plugin* createPlugin() { return new Ga88Plugin(); }
